...
// PBR / Caustics integration should never hard-crash the app if a module fails
  ...
  raycastSpa(event, dom = this.renderer?.domElement) {
    ...
    const meshHits = spaMeshes.length ? ray.intersectObjects(spaMeshes, true) : [];
    if (meshHits.length) return meshHits;

    // Fallback to a slightly expanded spa bounds hit test so direct-drag keeps
    // working even when thin geometry/debug overlays make mesh picking fiddly.
    this.spa.updateMatrixWorld?.(true);
    const spaBox = new THREE.Box3().setFromObject(this.spa).expandByScalar(0.15);
    const boxHit = ray.ray.intersectBox(spaBox, new THREE.Vector3());
    return boxHit ? [{ object: this.spa, point: boxHit, distance: ray.ray.origin.distanceTo(boxHit) }] : [];
  }
  ...
  setupSpaSelection() {
    ...
    window.addEventListener("pointermove", (event) => {
      if (!this.spaDrag?.active || !this.spa || this.selectedSpa !== this.spa) return;
      const point = this.intersectSpaDragPlane(event, dom);
      if (!point) return;
    ...
    window.addEventListener("pointerup", finishSpaDrag);
    window.addEventListener("pointercancel", finishSpaDrag);
