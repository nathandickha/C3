...
export function snapToPool(spa) {
  ...
  // Keep the original spa placement behaviour, but make the channel/void state
  // follow the top-offset slider more predictably. At 0.05m and above the spa
  // should be treated as channel-enabled even if the current pre-snap position
  // still falls into the inner-flush bucket.
  const topOffsetValue = spa?.userData?.topOffset ?? SPA_TOP_OFFSET;
  if (!isHalfwayInWall && topOffsetValue >= 0.05 - 1e-6) {
    isHalfwayInWall = true;
    snapVariant = disableExternalSnap
      ? "center-wall"
      : (topOffsetValue >= 0.10 - 1e-6 ? "inner-wall-align" : "center-wall");
  }
