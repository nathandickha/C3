...
export function snapToPool(spa) {
  ...
  // Make the spa snap state deterministic from the top-offset slider so the
  // channel/void behaviour matches the UI:
  //   0.00 -> flush against wall (no channel)
  //   0.05 -> halfway in wall (channel on)
  //   0.10+ -> offset outward / wall-align (channel on for square spas)
  // This avoids the previous situation where 0.05 and 0.10 depended on the
  // spa's current pre-snap position and could land in different snap buckets.
  const topOffsetValue = spa?.userData?.topOffset ?? SPA_TOP_OFFSET;
  const forceCenterSnap = topOffsetValue >= 0.05 - 1e-6;
  const forceOuterSnap = !disableExternalSnap && topOffsetValue >= 0.10 - 1e-6;

  const snapToOuter = forceOuterSnap || (!forceCenterSnap && !disableExternalSnap && Math.abs(normalDistance - outerDistance) <= SPA_WALL_CENTER_SNAP_THRESHOLD);
  const snapToCenter = !snapToOuter && (forceCenterSnap || Math.abs(normalDistance - centerDistance) <= SPA_WALL_CENTER_SNAP_THRESHOLD);
