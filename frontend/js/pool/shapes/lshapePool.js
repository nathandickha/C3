...
/* -------------------------------------------------------
  ...
  const copingAO = copingTexLoader.load(new URL("../../textures/Coping/TilesTravertine001_AO_4K.jpg", import.meta.url).href);
  const copingRough = copingTexLoader.load(new URL("../../textures/Coping/TilesTravertine001_GLOSS_4K.jpg", import.meta.url).href);
  const copingNormal = copingTexLoader.load(new URL("../../textures/Coping/TilesTravertine001_NRM_4K.jpg", import.meta.url).href);
  [copingCol, copingAO, copingRough, copingNormal].forEach((tex) => {
  ...
  function makeCopingMat() {
    return new THREE.MeshStandardMaterial({
      map: copingCol,
      aoMap: copingAO,
      roughnessMap: copingRough,
      normalMap: copingNormal,
      color: 0xffffff,
      metalness: 0.0,
      roughness: 1.0,
      side: THREE.DoubleSide
  ...
  for (let i = 0; i < pts2D.length; i++) {
    const p0 = pts2D[i];
    const p1 = pts2D[(i + 1) % pts2D.length];
    const dx = p1.x - p0.x;
    const dy = p1.y - p0.y;
    ...
    const copingGeo = isHorizontal
      ? new THREE.BoxGeometry(len, copingWidth, copingDepth)
      : new THREE.BoxGeometry(copingWidth, len, copingDepth);
